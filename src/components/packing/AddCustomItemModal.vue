...
const categoryOptions: { label: string; value: PackingCategory }[] = [
  { label: '🎒 背包系統 (PACK)', value: 'PACK' },
  { label: '👕 服飾穿搭 (CLOTHING)', value: 'CLOTHING' },
  { label: '👟 鞋襪足部 (FOOTWEAR)', value: 'FOOTWEAR' },
  { label: '🛏️ 睡眠防護 (SLEEP)', value: 'SLEEP' },
  { label: '🧼 衛浴清潔 (HYGIENE)', value: 'HYGIENE' },
  { label: '💊 醫藥防蟲 (MEDICAL)', value: 'MEDICAL' },
  { label: '🔌 電子電力 (ELECTRONICS)', value: 'ELECTRONICS' },
  { label: '📄 證件工具 (DOCS)', value: 'DOCS' },
  { label: '📦 其他輔助 (OTHER)', value: 'OTHER' },
]
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
      <DialogPanel class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl space-y-4">
        ...
        <form class="space-y-3.5 text-xs" @submit.prevent="handleSubmit">
          <div>
            <label class="block font-bold text-slate-700">裝備名稱 (中文必填)</label>
            <input
              v-model="formName"
              type="text"
          ...
          <div>
            <label class="block font-bold text-slate-700">英文品名 (選填)</label>
            <input
          ...
          <div>
            <label class="block font-bold text-slate-700">所屬裝備分類</label>
            <select
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block font-bold text-slate-700">單件實測重量 (克)</label>
              <input
            ...
            <div>
              <label class="block font-bold text-slate-700">數量</label>
              <input
          ...
          <div class="flex items-center justify-between rounded-xl bg-slate-50 p-3">
            <div>
              <div class="font-bold text-slate-800">當天穿戴在身上？</div>
          ...
          <div class="mt-4 flex justify-end gap-2 pt-2">
            ...
            <button
              type="submit"
              class="rounded-xl bg-amber-500 px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-amber-600 transition-colors"
            >
              確認加入清單
            </button>
          </div>
